...
- [ ] **[P1]** 统一组件状态同步机制 ⏳ **部分完成**
  ...
  - [ ] 改进表单提交逻辑
  - [ ] 编写集成测试
  - **文件**: `core/types.go`, `model.go`, `components/table.go`, `components/menu.go`
  - **预估时间**: 4-6 小时
...
- [ ] **[P1]** 完善错误处理和用户反馈 ⏳ **待实现**
  ...
  - [ ] 错误日志详细化
  - [ ] 用户友好的错误提示
  - **文件**: `render.go`, 新增 `error_component.go`
  - **预估时间**: 2-3 小时

- [ ] **[P1]** 分离 Render 和 UpdateProps 职责 ⏳ **待实现**
  - [ ] 统一组件的 `Render()` 为纯函数
  - [ ] 将状态更新逻辑完全移到 `UpdateRenderConfig()`
  - [ ] 更新所有组件实现
  - [ ] 编写测试验证状态更新时机
  - **文件**: 所有组件文件, `core/types.go`
  - **预估时间**: 3-4 小时
